...
实现了网络消息与RPC处理的统一机制。
- 主要结构：
  - `MessageType`：网络消息类型（交易、区块、区块请求等）。
  - `Message`：网络消息结构体，支持 gob 序列化。
  - `RPC`：远程过程调用消息结构。
  - `DecodedMessage`：解码后的消息结构体。
  - `RPCDecodeFunc`/`DefaultRPCDecodeFunc`：RPC消息解码函数类型与默认实现。
  - `RPCProcessor`：RPC处理器接口。
- 主要功能：
  - 网络消息的类型标识与序列化。
  - RPC消息的统一解码与分发。
  - 交易、区块等消息的解码与处理。
